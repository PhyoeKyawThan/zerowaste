...
if (isset($_GET['dId'])) {
    ...
    if (isset($_GET['del'])) {
        ...
        ?>
                    ...
                    <table class="table table-striped align-middle">
                        ...
                        <tbody>
                            ...
                            while ($row = mysqli_fetch_assoc($query)) {
                                ...
                                $status_class = $row['status'] == 'Pending' ? 'warning text-dark' : ($row['status'] == 'Approved' ? 'success' : 'danger');

                                $pend_selected = $row['status'] == 'Pending' ? 'selected' : '';
                                $approv_selected = $row['status'] == 'Approved' ? 'selected' : '';
                                $reject_selected = $row['status'] == 'Rejected' ? 'selected' : '';
                                echo '<select class="badge bg-' . $status_class . '  rounded" onchange="changeStatus(event, ' . $row['d_id'] . ')">
                                <option value="Pending" ' . $pend_selected . '>Pending</option>
                                <option value="Approved" ' . $approv_selected . '>Approved</option>
                                <option value="Rejected" ' . $reject_selected . '>Rejected</option>
                            </select>';
            ...
            <script>
                async function changeStatus(e, d_id) {
                    const response = await fetch(`/zerowaste/admin/actions/change_dish_status.php?status=${e.target.value}&id=${d_id}`,
                        {
                            method: "POST"
                        }
                    );
